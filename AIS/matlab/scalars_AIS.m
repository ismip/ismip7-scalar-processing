...
% AIS MATLAB implementation — mirrors scalars_AIS.py exactly
...
flg_bm = true;  % IMBIE3-Rignot basins and regions
...
ais = maxmask1 * 0 + 1; % full grid
regions = struct('mm', ais);

% IMBIE3 Rignot regions (1=WAIS, 2=EAIS, 3=PINA) and basins (1-18)
if flg_bm
    % 3 regions
    ...
    if verbose
        ...
        fprintf('ais sum=%g  region sum=%g  nx*ny=%d\n', ...
                sum(ais(:)), sum(aistest(:)), numel(ais));
    end
end
...
lithk_file = [exppath '/lithk_AIS_' lab '_' model '_' exp '.nc'];
...
topg_file = [exppath '/topg_AIS_' lab '_' model '_' exp '.nc'];
...
lithk_ref_all = double(ncread([histrefpath '/lithk_AIS_' lab '_' model '_' histref '.nc'], 'lithk'));
lithk_ref     = lithk_ref_all(:,:,end); % (nx, ny)
topg_ref_all  = double(ncread([histrefpath '/topg_AIS_'  lab '_' model '_' histref '.nc'], 'topg'));
...
sftgif = double(ncread([exppath '/sftgif_AIS_' lab '_' model '_' exp '.nc'], 'sftgif'));
sftgrf = double(ncread([exppath '/sftgrf_AIS_' lab '_' model '_' exp '.nc'], 'sftgrf'));
sftflf = double(ncread([exppath '/sftflf_AIS_' lab '_' model '_' exp '.nc'], 'sftflf'));
